        ...
        <div class="d-flex justify-content-between align-items-center mb-3">
            ...
            <div class="dropdown">
                ...
                <ul class="dropdown-menu dropdown-menu-end" aria-labelledby="dropdownMenuButton">
                    <li>
                        <a class="dropdown-item" href="{{ route('student.posters', ['sort_by' => 'most_liked']) }}">
        ...
        <div class="row">
            ...
            <div class="col-md-4 mb-4">
              <div class="card position-relative">
                      <!-- Menampilkan gambar -->
                      <img src="{{ asset($poster->file_path) }}" 
                           class="card-img-top img-fluid" 
                           alt="Media" 
                           onclick="showModal('{{ asset($poster->file_path) }}')">
                  
                  <!-- Ikon love di pojok kiri bawah -->
                  <div class="position-absolute bottom-0 start-0 p-2">
                    <a href="javascript:void(0)" class="like-btn" data-id="{{ $poster->id }}">
                        <i class="far fa-heart text-white {{ $poster->likes->contains('user_id', auth()->id()) ? 'liked fas' : '' }}" 
                  ...
                  <div class="position-absolute bottom-0 end-0 p-2">
                    <a href="javascript:void(0)" class="comment-btn" data-id="{{ $poster->id }}" data-bs-toggle="modal" data-bs-target="#commentModal">
              ...
              <div class="card-body">
                <div class="d-flex justify-content-between">
                  <!-- Title di kiri -->
                  <h5 class="card-title mb-0">{{$poster->title}}</h5>
                  <!-- Artwork by di kanan -->
                    <p style="font-size: 13px;" class="mb-0 text-end">created by: {{$poster->student->username}}</p>
                </div>
                <p class="card-text">{{$poster->description}}</p>
              </div>
              <div class="container">
                  <p class="d-flex justify-content-end">{{$poster->created_at->diffForHumans()}}</p>
              </div>
            </div>
    ...
    <div class="page-inner">
        ...
        <div class="row">
            ...
            <div class="card position-relative">
                ...
                @if (in_array(pathinfo($poster->file_path, PATHINFO_EXTENSION), ['mp4', 'avi', 'mov', 'mkv']))
                    <!-- Menampilkan video -->
                    <video controls class="card-img-top img-fluid" style="height: 400px; object-fit: cover;">
                        <source src="{{ asset($poster->file_path) }}" type="video/{{ pathinfo($poster->file_path, PATHINFO_EXTENSION) }}">
                ...
                <!-- Ikon opsi di pojok kanan atas -->
                    <div class="position-absolute top-0 end-0 p-2">
                        <div class="dropdown">
                            <a href="javascript:void(0)" class="options-btn" id="dropdownMenu{{ $poster->id }}" data-bs-toggle="dropdown" aria-expanded="false">
                            ...
                            <ul class="dropdown-menu dropdown-menu-end mt-3" aria-labelledby="dropdownMenu{{ $poster->id }}">
                                <!-- Edit -->
                                <li>
                                    <a href="javascript:void(0)" class="dropdown-item" data-bs-toggle="modal" data-bs-target="#editModal{{ $poster->id }}">
                                        <i class="far fa-edit" style="font-size: 1.5rem; padding-left: 1px;"></i> Edit
                                    </a>
                                </li>
                                <!-- Hapus -->
                                <li>
                                    <form action="{{ route('artwork.destroy', $poster->id) }}" method="POST" onsubmit="return confirm('Yakin ingin menghapus karya ini?');">
                                        @csrf
                                        @method('DELETE')
                                ...
                                <!-- Repost -->
                                <li>
                                    <form action="{{ route('artwork.repost', $poster->id) }}" method="POST" style="display: inline;">
                                        @csrf
                                        <button type="submit" class="dropdown-item">
                                            @if ($poster->is_reposted)
                    ...
                    <!-- Modal Edit -->
                    <div class="modal" id="editModal{{ $poster->id }}" tabindex="-1" aria-labelledby="editModalLabel{{ $poster->id }}" aria-hidden="true">
                        <div class="modal-dialog">
                            <div class="modal-content modal-slide-up-down"
                                ...
                                <div class="modal-header">
                                    <h5 class="modal-title" id="editModalLabel{{ $poster->id }}">Edit Karya Seni</h5>
                                    <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
                                </div>
                                <div class="modal-body">
                                    <form action="{{ route('artwork.update', $poster->id) }}" method="POST" enctype="multipart/form-data">
                                        @csrf
                                        @method('PUT')
                                        ...
                                        <div class="mb-3">
                                            ...
                                            <input type="text" name="title" class="form-control" id="title" value="{{ old('title', $poster->title) }}" required>
                                        </div>

                                        <!-- Description -->
                                        <div class="mb-3">
                                            <label for="description" class="form-label">Deskripsi</label>
                                            <textarea name="description" class="form-control" id="description">{{ old('description', $poster->description) }}</textarea>
                                        </div>
                                        ...
                                        <div class="mb-3">
                                            ...
                                            <select name="category" class="form-select" id="category" required>
                                                <option value="berita" {{ $poster->category == 'berita' ? 'selected' : '' }}>Berita</option>
                                                <option value="video" {{ $poster->category == 'video' ? 'selected' : '' }}>Video</option>
                                                <option value="puisi" {{ $poster->category == 'puisi' ? 'selected' : '' }}>Puisi</option>
                                                <option value="poster" {{ $poster->category == 'poster' ? 'selected' : '' }}>Poster</option>
                                            </select>
                                        </div>
                                        ...
                                        <div class="mb-3">
                                            ...
                                            <small>File yang sudah ada: <a href="{{ asset($poster->file_path) }}" target="_blank">Lihat File</a></small>
                                        </div>
